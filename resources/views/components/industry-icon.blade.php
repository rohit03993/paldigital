@props(['icon' => 'sparkles', 'class' => 'w-6 h-6 sm:w-7 sm:h-7'])
...
<svg {{ $attributes->merge(['class' => $class . ' text-pal-yellow']) }} fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
    @switch($icon)
        @case('academic-cap')
            ...
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.75" d="M12 14v7"/>
            @break
        @case('heart')
            {{-- Healthcare --}}
            ...
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.75" d="M12 8v4m0 0v4m0-4h4m-4 0H8"/>
            @break
        @case('building')
            {{-- Real Estate --}}
